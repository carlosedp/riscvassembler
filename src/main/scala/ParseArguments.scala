...
object ParseArguments {
  ...
  /** Parses command line arguments
    *
    * This function parses command line arguments from `args`, as an `Array[String]`. The parameters to be extracted can
    * be prefixed with `-` as default or the param prefix in `paramFormat` (for example `--`) and passed in `parameters`
    * as `List[String]`.
    *
    * The function returns a `Map[String, String]` with extracted parameters and a `List[String]` with remaining
    * arguments that were not extracted. The parameter name is returned with it's leading `paramFormat` (`"-"` for
    * example) removed.
    *
