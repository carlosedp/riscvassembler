...
```scala
// Import libraries
libraryDependencies += "com.carlosedp" %% "riscvassembler" % "1.9.1"  //ReleaseVerSBT
```
...
// Add to your ivyDeps
def ivyDeps = Agg(
  ivy"com.carlosedp::riscvassembler:1.9.1"  //ReleaseVerMill
  ...
)
```
...
To generate the tool binary yourself, use `./mill Alias/run bin` and the native executable will be generated and it's name printed on screen.
...
libraryDependencies += "com.carlosedp" %% "riscvassembler" % "1.10-SNAPSHOT"  //SnapshotVerSBT
```
...
def ivyDeps = Agg(
  ivy"com.carlosedp::riscvassembler:1.10-SNAPSHOT"  //SnapshotVerMill
  ...
)
```
